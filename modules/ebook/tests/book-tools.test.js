...
test('Delegate prompt gives the reviewer a stable book-specific tool model', () => {
    ...
    assert.match(EBOOK_DELEGATE_PROMPT, /# 审稿分身系统提示词/);
    assert.match(EBOOK_DELEGATE_PROMPT, /## 身份与边界/);
    assert.match(EBOOK_DELEGATE_PROMPT, /不是主助手的附和者/);
    assert.match(EBOOK_DELEGATE_PROMPT, /独立审稿人/);
    assert.match(EBOOK_DELEGATE_PROMPT, /保护这本书的生命力/);
    assert.match(EBOOK_DELEGATE_PROMPT, /当前打开的书是唯一工作对象/);
    assert.match(EBOOK_DELEGATE_PROMPT, /book\/chapters\//);
    assert.match(EBOOK_DELEGATE_PROMPT, /\[ebook-image:slotId\]/);
    assert.match(EBOOK_DELEGATE_PROMPT, /## 信息来源/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 自动注入（不要重复读取）/);
    assert.match(EBOOK_DELEGATE_PROMPT, /\[审稿分身自动上下文\]/);
    assert.match(EBOOK_DELEGATE_PROMPT, /\| `book\/outline\.md` \| 开书定位、故事脊柱、欲望链、全书骨架 \|/);
    assert.match(EBOOK_DELEGATE_PROMPT, /\| `book\/style\.md` \| 写法方案 \|/);
    assert.match(EBOOK_DELEGATE_PROMPT, /这些文件已经注入，直接作为判断依据，不要用 Read 重复读取/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 需要按需读取/);
    assert.match(EBOOK_DELEGATE_PROMPT, /`book\/volumes\/` 卷规划/);
    assert.match(EBOOK_DELEGATE_PROMPT, /审稿时必须 Read 对应章节原文/);
    ...
    assert.match(EBOOK_DELEGATE_PROMPT, /## 审稿标准层级/);
    assert.match(EBOOK_DELEGATE_PROMPT, /`book\/review-rules\.md`：本书的固定审稿标准，最高优先级/);
    assert.match(EBOOK_DELEGATE_PROMPT, /正文证据：实际写了什么，以文本为准/);
    assert.match(EBOOK_DELEGATE_PROMPT, /不以主助手给你的临时规范为准/);
    assert.match(EBOOK_DELEGATE_PROMPT, /用户写的 review-rules 和 `核心审稿观念` 为准/);
    assert.match(EBOOK_DELEGATE_PROMPT, /## 核心审稿观念/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 重审重点/);
    assert.match(EBOOK_DELEGATE_PROMPT, /第一优先抓任务式写作/);
    assert.match(EBOOK_DELEGATE_PROMPT, /只是在执行剧情/);
    assert.match(EBOOK_DELEGATE_PROMPT, /如果是章纲安排太满，应该改章纲/);
    assert.match(EBOOK_DELEGATE_PROMPT, /不是因为正文达到了章纲目标就判通过/);
    assert.match(EBOOK_DELEGATE_PROMPT, /第一优先抓解释式假冰山/);
    assert.match(EBOOK_DELEGATE_PROMPT, /某件即将是关键节点的物品/);
    assert.match(EBOOK_DELEGATE_PROMPT, /作者视角泄漏和 tell/);
    assert.match(EBOOK_DELEGATE_PROMPT, /禁止叙述者来自未来的视角/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 结构/);
    assert.match(EBOOK_DELEGATE_PROMPT, /欲望链引领结构/);
    assert.match(EBOOK_DELEGATE_PROMPT, /事件集团是叙事单位/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 节奏/);
    assert.match(EBOOK_DELEGATE_PROMPT, /短期欲望 -> 障碍 -> 行动 -> 结果/);
    assert.match(EBOOK_DELEGATE_PROMPT, /章末位移是结果，不是目标/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 人物/);
    assert.match(EBOOK_DELEGATE_PROMPT, /人物要像在生活/);
    assert.match(EBOOK_DELEGATE_PROMPT, /### 优先级/);
    assert.match(EBOOK_DELEGATE_PROMPT, /章节像任务清单，哪怕句子顺，也是严重问题/);
    assert.match(EBOOK_DELEGATE_PROMPT, /## 工作流程/);
    assert.match(EBOOK_DELEGATE_PROMPT, /只处理 `\[Task\]` 里的子任务/);
    assert.match(EBOOK_DELEGATE_PROMPT, /区分三类问题：硬伤/);
    assert.match(EBOOK_DELEGATE_PROMPT, /可以直接判定“不通过”或“需要重写”/);
    assert.match(EBOOK_DELEGATE_PROMPT, /## 输出格式/);
    assert.match(EBOOK_DELEGATE_PROMPT, /总体判断：通过 \/ 需修改 \/ 需重写/);
    assert.match(EBOOK_DELEGATE_PROMPT, /可执行修改建议：具体、可操作/);
