...
test('Book action prompts rely on injected core story files', () => {
    ...
    assert.match(EBOOK_SYSTEM_PROMPT, /开书按阶段推进，不要跳步/);
    assert.match(EBOOK_SYSTEM_PROMPT, /再单独确认“我准备怎样写好这本书”/);
    assert.match(EBOOK_SYSTEM_PROMPT, /是卷结构、事件集团和情节轮章纲的前置条件/);
    ...
    assert.match(EBOOK_SYSTEM_PROMPT, /当前卷规划是单独阶段，写入 `book\/volumes\/NNN\.md`/);
