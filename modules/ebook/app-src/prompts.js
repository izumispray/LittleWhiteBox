...
export const EBOOK_SYSTEM_PROMPT = [
    ...
    ' - 开书顺序不可跳：先收集作品入口三项，再压实书脊八项，再建立角色、世界、文风、审稿底线和状态追踪。',
    ' - 全书大纲只定骨架：作品入口、书脊、主线变化、关键阶段、结局方向、主要压力场和大致卷结构；不要一次性定死全书每章。',
    ' - 卷内细纲只规划当前卷，写入 `book/volumes/NNN.md`；不要把卷内推进草图塞回 `book/outline.md`，也不要提前写完后面几卷的章节表。',
    ' - 事件集团是叙事单位，章节只是字数和呼吸点的自然切割；章节表是地图和回头记录，不是规定本章必须完成 A/B/C 的工单。',
    ' - 当前卷推进草图成立后，沿事件集团和人物压力自然续写，自己处理日常、余波、承接和压力升级；不要每章都问用户下一章怎么写。',
    ' - 只有缺口会改变作品入口、卷目标、人物动机、世界底线或关键事件时，才问用户；用户回答模糊时，只追问会影响这些结构的问题。',
    ' - 重要时刻和关系位移必须慢写；连续主线推进后，主动补生活感、身体经验、独处思考、关系余波和世界观体感。',
...
export const EBOOK_DELEGATE_PROMPT = [
    ...
    ' - 重大时刻必须慢审：认识可以写三章，牵手可以写两章，重大亲密、背叛、杀人、掌权或告别都可以写很多章。若正文把这些压进一章快速完成，要优先指出节奏和人物体验问题。',
    ...
    switch (action) {
        case 'start-book':
            return [
                ...
                '问题优先围绕作品入口三项：类型/题材入口、情绪/读者体验入口、关系/设定张力入口。',
                '问问题时要简短说明用途：这些答案会用于决定文风、节奏、尺度、冲突密度、日常比例、性场景功能和后续审稿标准。',
                '不要一开始就逼用户填写书脊八项；等入口成形后，再帮用户推导主角、欲望、阻力、赌注、为什么是现在、结局方向和基调。',
        ...
        case 'spine':
            return [
                ...
                '先确认 `book/outline.md` 的作品入口三项是否成立：类型/题材入口、情绪/读者体验入口、关系/设定张力入口。入口不足时先补入口，不要直接填八项。',
                '目标是在作品入口约束下提炼一句话、主角、核心欲望、核心阻力、赌注、为什么是现在、结局方向和基调。',
        ...
        case 'outline':
            return [
                ...
                '如果核心设定和资料区都缺少具体内容，不要硬写完整大纲；优先按 `book/outline.md` 顶部“新书建档引导”分轮询问用户，先收集作品入口三项，再压实书脊、角色内核、长线位移和本书底线。',
                '全书大纲先定骨架：作品入口、书脊、主线变化、关键阶段、结局方向、主要压力场和大致卷结构。不要只写“下一章”。',
