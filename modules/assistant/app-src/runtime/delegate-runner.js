...
function normalizeDelegateArgs(args = {}) {
    return {
        ...
        context: normalizeText(args.context, 10000),
...
function normalizeToolCalls(toolCalls = []) {
    return (Array.isArray(toolCalls) ? toolCalls : [])
        .map((toolCall, index) => ({
            id: String(toolCall?.id || `delegate-tool-${Date.now()}-${index + 1}`),
...
function resolveDelegateToolCalls(result = {}, providerConfig = {}) {
    const directToolCalls = normalizeToolCalls(result?.toolCalls);
    ...
    return normalizeToolCalls(extractGoogleProviderToolCalls(result?.providerPayload));
}

function buildAssistantToolCallMessage(result = {}, toolCalls = []) {
    return {
        role: 'assistant',
...
export function createDelegateRunner(deps = {}) {
    ...
    async function runDelegate(args = {}, parentRun = {}) {
        ...
        while (rounds < maxRounds) {
            ...
            const toolCalls = resolveDelegateToolCalls(result, providerConfig);
            if (toolCalls.length) {
                pendingToolResponses = null;
                sawToolExecution = true;
                toolCallCount += toolCalls.length;
                messages.push(buildAssistantToolCallMessage(result, toolCalls));
