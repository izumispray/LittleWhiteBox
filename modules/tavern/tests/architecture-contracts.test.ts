...
test('tavern phone OS drives registered apps through a generic definition contract', () => {
    ...
    assert.match(phoneOsSource, /function launchApp/);
    assert.match(phoneOsSource, /function backOrClose/);
    ...
    assert.match(phoneControllerSource, /await messages\.markActiveThreadRead\(threadId\)/);
    assert.doesNotMatch(phoneControllerSource, /async function openPhone\(\) \{[\s\S]*?messages\.prepareMessages/);
    assert.match(communicationSource, /markTavernCommunicationThreadRead[\s\S]*db\.transaction\('rw', tavernCommunicationThreadsTable/);
    assert.match(overlaySource, /TavernPhoneHome/);
    assert.match(overlaySource, /TavernPhoneSystemNavigation/);
    assert.match(overlaySource, /TavernPhoneAppStage/);
...
test('tavern heavy disclosure details bind to ephemeral state instead of keeping bodies mounted', () => {
    ...
    const conversationSource = readRepoFile('modules/tavern/app-src/components/chat/TavernConversationPanel.vue');
    const managerSource = readRepoFile('modules/tavern/app-src/components/chat/TavernManagerPanel.vue');
    const assistantToolTurnSource = readRepoFile('modules/tavern/app-src/components/chat/TavernAssistantToolTurn.vue');
    ...
    assert.match(managerSource, /<TavernAssistantToolTurn[\s\S]*:open="assistantChatDisclosure\.isOpen/);
    assert.match(assistantToolTurnSource, /v-if="live \|\| open"/);
    ...
    assert.match(assistantToolTurnSource, /watch\(\[\(\) => props\.open, \(\) => props\.live\][\s\S]*openThoughtRounds\.value = new Set/);
