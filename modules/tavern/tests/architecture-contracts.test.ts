...
test('tavern streaming action-check UI renders from live runtime events and keeps dark card styling aligned', () => {
    ...
    assert.match(conversationPanelSource, /<TavernStreamMarkdown[\s\S]*:animated="true"[\s\S]*:html="renderRoleplayMarkdown\(liveAssistantRenderState\.text\)"[\s\S]*:signature="liveAssistantRenderState\.signature"/);
    ...
    assert.match(streamMarkdownSource, /animated\?: boolean/);
    assert.match(streamMarkdownSource, /:class="\{ 'xb-tavern-stream-fade': animated !== false \}"/);
    ...
    assert.match(streamFadeMarkdownSource, /const DIALOGUE_CLASS = 'xb-rp-dialogue';/);
    assert.match(streamFadeMarkdownSource, /'"': '"/);
    assert.match(streamFadeMarkdownSource, /dialogue\.className = DIALOGUE_CLASS;/);
    assert.match(streamFadeMarkdownSource, /allowOpenEnded: true/);
    assert.match(streamFadeMarkdownSource, /ranges\.push\(\{ start: cursor, end: text\.length \}\);/);
    assert.match(streamFadeMarkdownSource, /patchChildren\(root, target\);/);
    assert.match(streamFadeMarkdownSource, /catch \{[\s\S]*setHtmlContent\(root, html\);[\s\S]*\}/);
    ...
    assert.match(chatRunSource, /runtimeFinalizedAssistantMessageKey: ref\(''\)/);
    assert.doesNotMatch(chatRunSource, /runtimeFinalizedAssistantMessage(?!Key)/);
    ...
    assert.match(appSource, /const followChatStreamToBottomIfAtBottom = chatScrollPane\.followStreamToBottomIfAtBottom;/);
    ...
    assert.match(appSource, /watch\(\[\s*\(\) => runtimeText\.value,[\s\S]*\(\) => runtimeThoughtsSignature\.value,[\s\S]*\(\) => runtimeActionCheckSignature\.value,[\s\S]*const hasLiveStreamContent = !!\([\s\S]*if \(isRunning\.value && hasLiveStreamContent\) \{[\s\S]*followChatStreamToBottomIfAtBottom\(\);/);
    ...
    assert.match(assistantSavedCallback[0], /flushRuntimeStreamSnapshotNow\(\);[\s\S]*options\.touchSessionLocally\(sessionId, message\.createdAt\);[\s\S]*state\.runtimeFinalizedAssistantMessageKey\.value = `\$\{message\.sessionId\}:\$\{message\.order\}`;[\s\S]*options\.upsertLoadedSessionMessage\(message\);[\s\S]*clearRuntimeAssistantLiveState\(\);/);
    const clearLiveStateBlock = chatRunSource.match(/function clearRuntimeAssistantLiveState\(\) \{[\s\S]*?\n[ ]{4}\}/);
    assert.ok(clearLiveStateBlock);
    assert.doesNotMatch(clearLiveStateBlock[0], /runtimeFinalizedAssistantMessageKey/);
    assert.match(chatRunSource, /function clearRuntimeFinalizedAssistantMessageKey\(\) \{[\s\S]*state\.runtimeFinalizedAssistantMessageKey\.value = '';/);
    assert.match(appSource, /function clearRuntimeFinalizedAssistantMessageKey\(\) \{[\s\S]*chatRunController\.clearRuntimeFinalizedAssistantMessageKey\(\);/);
    assert.match(appSource, /watch\(\[\(\) => activeView\.value, \(\) => chatFocus\.value\][\s\S]*wasRoleplayChat && !isRoleplayChat[\s\S]*clearRuntimeFinalizedAssistantMessageKey\(\);/);
    const activeFocusWatcher = appSource.match(/watch\(\[\(\) => activeView\.value, \(\) => chatFocus\.value\], \(\[view, focus\], \[previousView, previousFocus\]\) => \{[\s\S]*?\n\}\);/);
    assert.ok(activeFocusWatcher);
    assert.doesNotMatch(activeFocusWatcher[0], /placeChatAtBottomForNewContext|jumpChatToBottom|scrollToBottom/);
    assert.match(appSource, /watch\(\(\) => selectedSessionId\.value, \(\) => \{[\s\S]*clearRuntimeFinalizedAssistantMessageKey\(\);/);
    ...
    assert.match(chatRunSource, /options\.setSelectedSessionId\(result\.sessionId\);[\s\S]*flushRuntimeStreamSnapshotNow\(\);[\s\S]*clearRuntimeAssistantLiveState\(\);[\s\S]*await options\.refreshSessions\(\);\s*options\.updateChatScrollButtons\(\);/);
    assert.doesNotMatch(chatRunSource, /await options\.refreshSessions\(\);\s*if \(options\.chatAutoScroll\.value !== false\) \{[\s\S]*(?:scrollChatToBottom|placeChatAtBottomForNewContext|jumpChatToBottom)\(/);
    ...
    assert.match(conversationPanelSource, /const runtimeAssistantMessageItemDomKey = 'runtime-assistant-message-item'/);
    assert.match(conversationPanelSource, /function chatMessageItemDomKey\(message: TavernMessageRecord\)[\s\S]*message\.role === 'assistant' && messageKey\(message\) === runtimeFinalizedAssistantMessageKey\.value[\s\S]*runtimeAssistantMessageItemDomKey/);
    assert.match(conversationPanelSource, /function isRuntimeFinalizedAssistantMessage\(message: TavernMessageRecord\)[\s\S]*message\.role === 'assistant' && messageKey\(message\) === runtimeFinalizedAssistantMessageKey\.value;/);
    assert.match(conversationPanelSource, /<TavernStreamMarkdown[\s\S]*v-if="isRuntimeFinalizedAssistantMessage\(message\)"[\s\S]*:animated="false"[\s\S]*:html="renderRoleplayMarkdown\(render\.text\)"[\s\S]*:signature="render\.signature"/);
    assert.match(conversationPanelSource, /v-for="message in visibleChatMessages"[\s\S]*:key="chatMessageItemDomKey\(message\)"[\s\S]*class="chat-message-item"/);
    assert.match(conversationPanelSource, /v-if="liveAssistantCanRender && liveAssistantVisible"[\s\S]*:key="runtimeAssistantMessageItemDomKey"[\s\S]*class="chat-message-item"/);
    ...
    assert.match(chatPageSource, /watch\(chatScrollAnchorSignature, \(\) => \{[\s\S]*pendingChatScrollSnapshot = chatPaneVisible\.value[\s\S]*captureChatScrollSnapshot\(\)[\s\S]*\}, \{ flush: 'sync' \}\);/);
    assert.match(chatPageSource, /watch\(chatScrollAnchorSignature, \(\) => \{[\s\S]*restoreChatScrollSnapshot\(pendingChatScrollSnapshot, \{[\s\S]*preserveScrollTop: true,[\s\S]*\}, \{ flush: 'post' \}\);/);
    ...
    assert.match(cssSource, /\.chat-bubble\.streaming \{\s*contain: layout style;\s*\}/);
    assert.doesNotMatch(cssSource, /\.chat-bubble\.streaming \{[^}]*border-left-color:/);
    assert.match(cssSource, /\.chat-bubble\.streaming\.thinking \{[\s\S]*border-left-color: var\(--xb-cyan\);/);
    assert.match(markdownCssSource, /\.xb-tavern-stream-fade \.xb-tavern-stream-segment \{[\s\S]*animation: xb-tavern-stream-fade-in 300ms ease-in-out both;/);
    assert.doesNotMatch(markdownCssSource, /\.chat-bubble\.streaming \.xb-tavern-stream-segment/);
