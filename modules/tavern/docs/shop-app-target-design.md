...
10. 输入参数必须先按声明做 NFKC、空白折叠和限长，再经受控占位符进入人工审核的叙事正文；运行时只替换静态模板中的玩家与输入槽位，不得对替换后的用户数据做二次文本处理。
...
interface TavernShopItem {
    ...
    name: string;
    icon: string;
    category: 'emotion' | 'memory' | 'information' | 'behavior' | 'scene' | 'ultimate' | 'world-cognition' | 'physics';
    price: number;
    description: string;
...
- 若商品定义了 `deactivationInjection`，结束坐标对应的下一主 RP 会投影一次该转换；岁月之门据此回到主时间线。
...
<artifact_effects>
【沈默】使用了道具「吐真剂」，作用于【艾琳】。艾琳无法说出任何谎言，被问及任何事都必须说出真实想法。
</artifact_effects>
```
...
| `weather-call` | 呼风唤雨 | 200 | 1 回合 | 天气描述 | 按天象 |

完整 injection 必须在 `shop-catalog.ts`逐条人工编写和评审。商品说明可以简短，injection 必须明确作用对象、强制程度、持续边界和不得误改过去事实。

injection 措辞按道具的乐趣分类：

- 事件类（花、礼盒、记忆顺滑剂、记忆橡皮擦）：刚发生的一次性事件，命令式无妨。
- 状态类（不生气贴纸、崇拜滤镜、嫉妒种子、言听计从）：写成「ta 就是这样的人」，把强制内化为性情；不写「无法做 X」「必须服从」这类外力压制句式，避免模型演出「挣扎→屈服」的戏码。吐真剂是例外：「想瞒瞒不住」的对抗感正是它的乐趣，保留可感的强制力。
- 法则类（身份卡、反转贴纸、隐身斗篷、言出法随）：写成「世界本来就是这样」，不出现生效瞬间、改变痕迹或异样感；用正向描述给出表演基准（如「与玩家不在场时完全一致」），而不是罗列不能干什么。

## 10. UI 目标

### 10.1 视觉方向：星图奇物商店

商店是收藏和启用“改变世界的小器物”的陈列柜，而不是旧式当铺：

- 浅色：雾蓝瓷面、深海墨、铜金标记；深色：午夜蓝玻璃、冷蓝高光、暖金细节。两者是同一设计语言的明暗两面。
- 商品卡是有内发光与细窄渐变边的奇物陈列格；图标放进独立的器物底座，价格是紧凑的金色读数。
- 生效实例使用亮边状态卡；永久效果转为金色细节，不使用盖章、封契或齿边票据。
- 不使用旧纸、朱印、传统当铺符号，也不套用通用电商/SaaS 卡片。
