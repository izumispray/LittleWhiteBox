...
images:
  - index: 1
    ...
    scene: "comma-separated positive prompt: rating if relevant, subject count, composition, camera, background, lighting, mood"
    characters:
...
- Do not output scene-level negative prompts.
- Do not invent model, sampler, LoRA, VAE, ControlNet, workflow nodes, script, scheduler, seed, or extension settings.
